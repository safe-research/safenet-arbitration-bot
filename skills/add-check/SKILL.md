...
A check is a `check` value in `internal/checks` that decides a class of Safenet requests by direct application of the Charter, without judgment. It has the verdict for its class, the rule ID for an `insecure` verdict, a short description of the class, and a function that reports whether a request is in the class. `arbot classify` runs the checks in the `checks` variable in `internal/checks/checks.go`, the `out-of-scope` ones first, then the `insecure` ones, then the `secure` ones, and returns the classification of the first check that matches. `arbot classify -list` lists them in that order. Requests that every check abstains on go to the LLM stage, so a check that abstains when in doubt costs little, and one that decides wrongly misleads the Council.

## Inputs

Before writing any code, make sure you have:

- **The case**: the class of transactions the check decides, and the verdict it returns for them: `insecure` under a rule ID such as `R-4.1`, `out-of-scope` under § 3.9, or `secure`.
...
- **`secure`**: a transaction is secure only if it passes every rule of Article IV, so a check returns `secure` only for a class of transactions that passes all of them. An example is a Safe transaction that only calls `signMessage` and pays no relayer refund. A check that only shows that its own rule isn't broken must abstain.
...
A check receives only the request, as `arbot info -json` shows it: its terms, its proposal and Safe transaction, and its votes. If the case needs anything else, such as the Safe's owners or code at `safeBlock`, stop and agree with the user on how to extend the `check` type first. Any such data must come from the Safe's chain at `safeBlock`, or from Ethereum Mainnet at `ethereumBlock`, and never from later blocks (§ 2.8 and § 3.6).
...
1. **Write the check** in its own file, `internal/checks/<name>.go`, where `<name>` is a lowercase, underscored name for what it decides, such as `settings_change.go`. Declare it as a `check` variable, with a comment that quotes or paraphrases the rule, cites its ID and sections, and names the Charter version it was written against (its IPFS CID). Its fields are:
   ...
   - `fn`: a function that reports whether a request is in the class. It returns `false` whenever the request isn't clearly in the class, so that the check abstains. It returns an error only when it can't read the request, never to express a verdict. It decodes call data with `internal/solabi`, and compares addresses and hashes as `ethrpc` values rather than strings.
...
3. **Test the check** in `internal/checks/<name>_test.go`, with a table of requests that only set the fields the check reads. Cover:
